...
from app.models import StreamEventMessage, WhatIfConversation, WhatIfMessage
from app.services import AnalysisService
from app.utils.sse_stream_event_queue import SSEStreamEventQueue
from app.what_if_chat import (
    ConversationContext,
    WhatIfChatWorkflow,
...
class WhatIfWorkflowExecutorService:
    ...
    async def _try_get_conversation_context(
        self, conversation_id: str, analysis_id: str, owner_id: str
    ) -> ConversationContext:
        """Retrieve conversation context (e.g., message history)"""
        conversation: WhatIfConversation = (
            await self.what_if_message_repository.get_conversation_by_id(
        ...
        if not conversation:
            ...
            conversation_context = ConversationContext(
                conversation_id=conversation_id, message_history=[]
            )

        elif conversation.messages and len(conversation.messages) > 0:
            history_messages = sorted(
                conversation.messages, key=lambda msg: msg.sequence_number
            )
            conversation_context = ConversationContext(
                conversation_id=conversation_id,
                message_history=[
                    ChatMessage(role=msg.role, text=msg.text, author_name=msg.author)
    ...
    async def try_persist_conversation_message(
        ...
        try:
            ...
            logger.error(
                f"Failed to persist conversation message for conversation {conversation_id}: {str(e)}"
            )
            logger.exception(e)
    ...
    async def _handle_event(
        ...
        await sse_event_queue.add_event(
            StreamEventMessage(
                type=message_type, executor=executor, data=data, message=message
            )
        )
    ...
    async def execute_workflow(
        ...
        try:
            logger.info(
                f"Starting workflow execution for conversation {conversation_id}"
            )
            ...
            conversation_context = await self._try_get_conversation_context(
                conversation_id=conversation_id,
                analysis_id=analysis_id,
                owner_id=owner_id,
            )
            ...
            next_seq_num = (
                conversation_context.message_history is not None
                and len(conversation_context.message_history) + 1
                or 1
            )
            ...
            input = WhatIfChatWorkflowInputData(
                ...
                input_messages=ChatMessage(
                    role="user", text=input_message, author_name="User"
                ),
            )
            ...
            logger.info(
                f"Workflow execution completed for conversation {conversation_id}"
            )
    ...
    async def list_conversations(
        self, analysis_id: str, page: int = 1, page_size: int = 10
    ) -> List[WhatIfConversation]:
        """List all conversation IDs with pagination"""
        conversations = await self.what_if_message_repository.list_conversations(
            analysis_id=analysis_id, page=page, limit=page_size
        )
